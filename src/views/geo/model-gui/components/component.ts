...
export class Component {
  ...
  public getMarkUp(): Markup {
    ...
    // Diagram类型，并且restriction为block或者model，不用添加可连接点，
    // 区别是有没有自己添加可连接（magnet）markup

    // if (this.isViewCom) {
    // } else {
    //   subShapes = this.componentInfo.getConnectorSubShape();
    // }
  ...
  public createNode() {
    ...
    const isConnector =
      this.componentType === ViewType.Icon ||
      this.componentInfo.rawComponentInfo?.restriction?.includes('connector');
    // 有markup障碍物边界显示不对
    const item = {
      id,
      x: position.x,
      ...
      data,
      zIndex: isConnector
        ? ShapeLayer.ComponentPortZIndex
        : ShapeLayer.ComponentZIndex,
      attrs: {
        'node-corner': {
          fill: 'transparent',
          stroke: 'transparent'
    ...
    if (isConnector) {
      markup.forEach(item => {
        if (ConnectorShape.includes(item.tagName)) {
          item.attrs.magnet = true;
        }
      });
      item.ports = {
        items: [
  ...
  public getNodePosition() {
    ...
    return {
      x: Math.round(viewCenter.x * parentViewSx * parentFlipX + parentCenter.x),
      y: Math.round(viewCenter.y * parentViewSY * parentFlipY + parentCenter.y)
    };
  }
